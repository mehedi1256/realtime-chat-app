...
- Create groups with name, optional avatar, and initial members
- Group creator is admin; admin can add/remove members; any member can leave
- Real-time group messaging with encrypted messages (shared group key)
- Group typing indicator and last message preview in sidebar
- Group voice and video calls: one user starts a call, all members get an incoming notification; members can join or reject; WebRTC mesh between participants
- Member list with online/offline status; add-members modal (admin only)
...
|---|---|---|
| `_id` | ObjectId | Auto-generated unique identifier |
| `groupName` | String | Required, max 100 characters |
| `groupAdmin` | ObjectId (ref: User) | The group creator / admin |
| `groupMembers` | Array of ObjectId (ref: User) | Members of the group |
| `groupAvatar` | String | Optional path to group image |
| `createdAt` | Date | Auto-generated by Mongoose timestamps |
| `updatedAt` | Date | Auto-generated by Mongoose timestamps |

**Indexes:** `{ groupMembers: 1 }`, `{ groupAdmin: 1 }`
...
|---|---|---|
| `_id` | ObjectId | Auto-generated unique identifier |
| `groupId` | ObjectId (ref: Group) | The group this message belongs to |
| `sender` | ObjectId (ref: User) | The user who sent the message |
| `encryptedMessage` | String | AES-encrypted content (shared group key) |
| `fileUrl` | String | Optional path to attachment |
| `fileName` | String | Original filename |
| `fileType` | String | MIME type |
| `seenBy` | Array of ObjectId (ref: User) | Users who have seen this message |
...
| Method | Endpoint | Auth | Body / Params | Description |
|---|---|---|---|---|
| POST | `/api/groups` | Yes | `groupName`, `memberIds` (JSON array), `groupAvatar` (file, optional) | Create a group. Creator is admin. Returns populated group. |
| GET | `/api/groups` | Yes | -- | List groups the current user is in. Includes last message. |
| GET | `/api/groups/:groupId` | Yes | -- | Get a single group (members only). Populated admin and members. |
| PUT | `/api/groups/:groupId` | Yes | `groupName`, `groupAvatar` (file, optional) | Update group name/avatar (admin only). |
| POST | `/api/groups/:groupId/members` | Yes | `{ memberIds: string[] }` | Add members (admin only). Returns updated group and `added` ids. |
| DELETE | `/api/groups/:groupId/members/:userId` | Yes | -- | Remove a member (admin only). |
| POST | `/api/groups/:groupId/leave` | Yes | -- | Leave the group. If admin leaves, next member becomes admin. |
| GET | `/api/groups/:groupId/messages` | Yes | `?page=1&limit=50` | Get paginated group messages. |
...
### Group Chat and Group Call Events

| Event | Emitted By | Received By | Payload | Description |
|---|---|---|---|---|
| `join_group` | Client | Server | `groupId` | Join socket room for the group. Server verifies membership. |
| `leave_group` | Client | Server | `groupId` | Leave the group socket room. |
| `send_group_message` | Client | Server | `{ groupId, encryptedMessage?, fileUrl?, fileName?, fileType? }` | Send a group message. Server saves and broadcasts `receive_group_message` to room. |
| `receive_group_message` | Server | All in group room | Populated GroupMessage | New group message. |
| `group_typing` | Client | Server -> Group | `{ groupId }` | Server broadcasts `group_typing` to others in room. |
| `group_stop_typing` | Client | Server -> Group | `{ groupId }` | Server broadcasts `group_stop_typing`. |
| `group_members_added` | Client | Server | `{ groupId, group, addedIds }` | After adding members via API; server broadcasts `group_updated` to room and `added_to_group` to new members. |
| `group_call_start` | Client (initiator) | Server -> Group | `{ groupId, type }` | Start a group call. Server broadcasts `incoming_group_call` to other members. |
| `incoming_group_call` | Server | Other members | `{ groupId, from, type }` | Incoming group call notification. |
| `join_group_call` | Client | Server -> Group call room | `{ groupId }` | Join the call. Server broadcasts `group_call_participant_joined`. |
| `webrtc_offer` | Client | Server -> Target | `{ groupId, toUserId, offer }` | Relay SDP offer to another participant. |
| `webrtc_answer` | Client | Server -> Target | `{ groupId, toUserId, answer }` | Relay SDP answer. |
| `webrtc_ice_candidate` | Client | Server -> Target | `{ groupId, toUserId, candidate }` | Relay ICE candidate. |
| `leave_group_call` | Client | Server | `{ groupId }` | Leave the call. Server notifies others. |
| `end_group_call` | Client (initiator) | Server -> Call room | `{ groupId }` | End the call for everyone. Server emits `group_call_ended`. |
